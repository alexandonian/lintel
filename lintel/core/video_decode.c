 ...
 */
static int32_t receive_frame(struct video_stream_context *vid_ctx) {
    AVPacket packet;
    int32_t status;
    ...
    status = avcodec_receive_frame(vid_ctx->codec_context, vid_ctx->frame);
    if (status == 0)
    ...
    while (!was_frame_received &&
        ...
        if (packet.stream_index == vid_ctx->video_stream_index) {
            status = avcodec_send_packet(vid_ctx->codec_context, &packet);
            if (status != 0) {
 ...
 */
static AVFrame *allocate_rgb_image(AVCodecContext *codec_context) {
    int32_t status;
    AVFrame *frame_rgb;
    ...
    status =
        av_image_alloc(frame_rgb->data, frame_rgb->linesize, frame_rgb->width,
                       frame_rgb->height, AV_PIX_FMT_RGB24, 32);
    if (status < 0) {
 ...
 */
static uint32_t
copy_next_frame(uint8_t *dest, AVFrame *frame, AVFrame *frame_rgb,
                AVCodecContext *codec_context, struct SwsContext *sws_context,
                uint32_t copied_bytes, const uint32_t bytes_per_row) {
    ...
    for (int32_t row_index = 0; row_index < frame_rgb->height; ++row_index) {
 ...
 */
static void loop_to_buffer_end(uint8_t *dest, uint32_t copied_bytes,
                               int32_t frame_number, uint32_t bytes_per_frame,
...
void decode_video_to_out_buffer(uint8_t *dest,
    ...
    for (int32_t frame_number = 0; frame_number < num_requested_frames;
         ++frame_number) {
        int32_t status = receive_frame(vid_ctx);
        if (status == VID_DECODE_EOF) {
            loop_to_buffer_end(dest, copied_bytes, frame_number, bytes_per_frame,
        ...
        copied_bytes =
            copy_next_frame(dest, vid_ctx->frame, frame_rgb, codec_context,
                            sws_context, copied_bytes, bytes_per_row);
    }
...
int32_t read_memory(void *opaque, uint8_t *buffer, int32_t buf_size_bytes) {
    ...
    int32_t bytes_remaining =
        (input_buf->total_size_bytes - input_buf->offset_bytes);
    if (bytes_remaining < buf_size_bytes)
        buf_size_bytes = bytes_remaining;

    memcpy(buffer, input_buf->ptr + input_buf->offset_bytes, buf_size_bytes);
...
int64_t seek_memory(void *opaque, int64_t offset64, int32_t whence) {
    ...
    switch (whence) {
        case SEEK_CUR:
            ...
            break;
        case SEEK_END:
            input_buf->offset_bytes = (input_buf->total_size_bytes - offset);
            break;
        case SEEK_SET:
            input_buf->offset_bytes = offset;
            break;
 ...
 */
static AVInputFormat *probe_input_format(struct buffer_data *input_buf,
                                         const uint32_t buffer_size) {
    const int32_t probe_buf_size_bytes = (buffer_size + AVPROBE_PADDING_SIZE);
    AVProbeData probe_data = {NULL, (uint8_t *)av_malloc(probe_buf_size_bytes),
                              probe_buf_size_bytes, NULL};
 ...
 */
static int32_t find_video_stream_index(AVFormatContext *format_context) {
    AVStream *video_stream;
    uint32_t stream_index;

    for (stream_index = 0; stream_index < format_context->nb_streams;
         ++stream_index) {
...
int32_t setup_format_context(AVFormatContext **format_context_ptr,
    ...
    int32_t status = avformat_open_input(format_context_ptr, "", NULL, NULL);
    if (status < 0) {
        fprintf(stderr, "AVERROR: %d, message: %s\n", status,
#ifdef __cplusplus
                "");
#else
                av_err2str(status));
...
int64_t seek_to_closest_keypoint(float *seek_distance_out,
                                 struct video_stream_context *vid_ctx,
   ...
   */
    ...
    int64_t valid_seek_frame_limit = (vid_ctx->nb_frames - num_requested_frames);
   ...
   */
    ...
    timestamp = av_rescale_rnd(timestamp, vid_ctx->duration, vid_ctx->nb_frames,
                               AV_ROUND_DOWN);
   ...
   */
    ...
    int32_t status =
        av_seek_frame(vid_ctx->format_context, vid_ctx->video_stream_index,
...
int32_t skip_past_timestamp(struct video_stream_context *vid_ctx,
                            int64_t timestamp) {
...
void decode_video_from_frame_nums(uint8_t *dest,
     ...
     */
        int32_t avg_frame_duration = (vid_ctx->duration / vid_ctx->nb_frames);
        int64_t timestamp = frame_numbers[0] * avg_frame_duration;
        status = av_seek_frame(vid_ctx->format_context, vid_ctx->video_stream_index,
                               timestamp, AVSEEK_FLAG_BACKWARD);
        assert(status >= 0);
    ...
    for (; out_frame_index < num_requested_frames; ++out_frame_index) {
        ...
        if (desired_frame_num > vid_ctx->nb_frames) {
            loop_to_buffer_end(dest, copied_bytes, out_frame_index, bytes_per_frame,
                               num_requested_frames);
            goto out_free_frame_rgb_and_sws;
        }
